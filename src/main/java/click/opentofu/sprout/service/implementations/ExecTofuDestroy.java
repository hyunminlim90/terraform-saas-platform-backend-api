...
@Slf4j
public class ExecTofuDestroy implements AsyncServiceSingle {
        ...
        return asyncWorkerSupply(
            () -> {
                ...
                try {
                    ...
                    if (!acquired) {
                        String msg = "❌ tofu destroy queue is full. Please try again later.";
                        emitter.send(SseEmitter.event().data(msg));
                        throw new RuntimeException("exec_tofu_destroy_semaphore_queue_is_full");
                    }

                    log.info("---------------------------------------------------------------------------");
                    log.info("AsyncServiceSingle-started for Tofu init, destroy Command execution.");
                    log.info("---------------------------------------------------------------------------");
                    ...
                    processUtils.execTofuCommand(
                        Arrays.asList("tofu", "destroy", "-auto-approve"),
                        workingDirPath,
                        emitter
                    );
                    ...
                    log.info("----------------------------------------------------------------------------------------");
                    log.info("Completed Tofu init, destroy Command execution. Requester : " + authEmailId);
                    log.info("----------------------------------------------------------------------------------------");
