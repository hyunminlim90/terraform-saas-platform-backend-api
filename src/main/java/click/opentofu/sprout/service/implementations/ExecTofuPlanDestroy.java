...
@Slf4j
public class ExecTofuPlanDestroy implements AsyncServiceSingle {
    ...
    public <T> CompletableFuture<Object> workerSingleSupplyAsync(T dto, SseEmitter emitter) {
        ...
        return asyncWorkerSupply(
            () -> {
                ...
                RSemaphore semaphore = semaphoreProvider.getDestroySemaphore();
                ...
                try {
                    ...
                    if (!acquired) {
                        String msg = "❌ tofu plan destroy queue is full. Please try again later.";
                        emitter.send(SseEmitter.event().data(msg));
                        throw new RuntimeException("exec_tofu_plan_destroy_semaphore_queue_is_full");
                    }

                    log.info("---------------------------------------------------------------------------");
                    log.info("AsyncServiceSingle-started for Tofu init, plan -destroy Command execution.");
                    log.info("---------------------------------------------------------------------------");
                    ...
                    processUtils.execTofuCommand(
                        Arrays.asList("tofu", "plan", "-destroy"),
                        workingDirPath,
                        emitter
                    );
                    ...
                    log.info("----------------------------------------------------------------------------------------");
                    log.info("Completed Tofu init, plan -destroy Command execution. Requester : " + authEmailId);
                    log.info("----------------------------------------------------------------------------------------");
