...
@Slf4j
public class ExecTofuPlan implements AsyncServiceSingle {
    ...
    public <T> CompletableFuture<Object> workerSingleSupplyAsync(T dto, SseEmitter emitter) {
        ...
        return asyncWorkerSupply(
            () -> {
                ...
                RSemaphore semaphore = semaphoreProvider.getPlanSemaphore();
                ...
                try {
                    ...
                    if (!acquired) {
                        String msg = "❌ tofu plan queue is full. Please try again later.";
                        emitter.send(SseEmitter.event().data(msg));
                        throw new RuntimeException("exec_tofu_plan_semaphore_queue_is_full");
                    }

                    log.info("-----------------------------------------------------------------");
                    log.info("AsyncServiceSingle-started for Tofu init, plan Command execution.");
                    log.info("-----------------------------------------------------------------");
                    ...
                    log.info("----------------------------------------------------------------------------------------");
                    log.info("Completed Tofu init, plan Command execution. Requester : " + authEmailId);
                    log.info("----------------------------------------------------------------------------------------");
